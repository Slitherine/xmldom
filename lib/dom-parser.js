 ...
 * @property {boolean} [errorsAsTextNodes]
 * Allows the parser to ignore syntax errors and fall back to parsing invalid
 * input as (possibly denormalized) text nodes.
...
function DOMParser(options) {
	 ...
	 * @readonly
	 */
	this.errorsAsTextNodes = !!options.errorsAsTextNodes;
}
...
DOMParser.prototype.parseFromString = function (source, mimeType) {
	...
	sax.parse(this.normalizeLineEndings(String(source)), defaultNSMap, entityMap, this.errorsAsTextNodes);
	if (!domBuilder.doc.documentElement) {
		// non-fatal when errorsAsTextNodes is true
		if (!this.errorsAsTextNodes) {
...
function position(locator, node, starts, ends) {
...
DOMHandler.prototype = {
	...
	startElement: function (namespaceURI, localName, qName, attrs, startOffset) {
		var doc = this.doc;
		var el = doc.createElementNS(namespaceURI, qName || localName);
		//el.starts = startOffset;
		var len = attrs.length;
		appendElement(this, el);
		this.currentElement = el;

		if (this.locator) position(this.locator, el, startOffset, undefined);
		for (var i = 0; i < len; i++) {
			...
			if (this.locator) {
				var attrLocator = attrs.getLocator(i);
				position(attrLocator, attr, attrLocator.starts, attrLocator.ends);
			}
			attr.value = attr.nodeValue = value;
			el.setAttributeNode(attr);
		}
	},
	endElement: function (namespaceURI, localName, qName, startOffset, endOffset) {
		if (this.locator) position(undefined, this.currentElement, undefined, endOffset);
		//this.currentElement.ends = endOffset;
		this.currentElement = this.currentElement.parentNode;
	},
	startPrefixMapping: function (prefix, uri) {},
	endPrefixMapping: function (prefix) {},
	processingInstruction: function (target, data, startOffset, endOffset) {
		var ins = this.doc.createProcessingInstruction(target, data);
		if (this.locator) position(this.locator, ins, startOffset, endOffset);
		//ins.starts = startOffset;
		//ins.ends = endOffset;
		appendElement(this, ins);
	},
	ignorableWhitespace: function (ch, start, length) {},
	characters: function (chars, start, length, startOffset, endOffset) {
		chars = _toString.apply(this, arguments);
		//console.log(chars)
		if (chars) {
			if (this.cdata) {
				var charNode = this.doc.createCDATASection(chars);
				//charNode.starts = start - 9; // '<![CDATA['.length
				//charNode.ends = start + length + 3; // ']]>'.length
			} else {
				var charNode = this.doc.createTextNode(chars);
				//charNode.starts = start;
				//charNode.ends = start + length;
			}
			...
			if (this.locator) {
				if (this.cdata) {
					position(this.locator, charNode, startOffset, endOffset);
				} else {
					position(this.locator, charNode, startOffset, endOffset);
				}
			}
	...
	//LexicalHandler
	comment: function (chars, start, length, end) {
		chars = _toString.apply(this, arguments);
		var comm = this.doc.createComment(chars);
		//comm.starts = start;
		//comm.ends = start + length;
		if (this.locator) position(this.locator, comm, end - length - 7, end);
	...
	startDTD: function (name, publicId, systemId, internalSubset, startOffset, endOffset) {
		var impl = this.doc.implementation;
		if (impl && impl.createDocumentType) {
			var dt = impl.createDocumentType(name, publicId, systemId, internalSubset);
			//dt.starts = startOffset;
			if (this.locator) position(this.locator, dt, startOffset, endOffset);
