...
XMLReader.prototype = {
	...
	parse: function (source, defaultNSMap, entityMap, errorsAsTextNodes) {
		var domBuilder = this.domBuilder;
		domBuilder.startDocument();
		_copy(defaultNSMap, (defaultNSMap = Object.create(null)));
		parse(source, defaultNSMap, entityMap, domBuilder, this.errorHandler, errorsAsTextNodes);
...
function parse(source, defaultNSMapCopy, entityMap, domBuilder, errorHandler, errorsAsTextNodes) {
	...
	function appendText(end) {
		...
		if (end > start) {
			...
			if (locator) position(start);
			domBuilder.characters(xt, 0, end - start, start, end);
			start = end;
		}
	}
	...
	function position(p, m) {
		while (p >= lineEnd && (m = linePattern.exec(source))) {
			lineStart = m.index;
			var fullMatch = m[0];
			lineEnd = lineStart + fullMatch.length;
			locator.lineNumber++;
			if (fullMatch[fullMatch.length - 1] !== '\n') {
				// no line end at end of input
				break;
			}
		}
		...
		try {
			...
			if (tagStart > start) {
				...
				if (!isHTML && unclosedTags.length === 0) {
					...
					fromSource &&
						fromSource.length !== 0 &&
						errorHandler.error("Unexpected content outside root element: '" + fromSource + "'");
				}
				appendText(tagStart);
			}
			// this loop has a break statement at the end
			// it is used to back up when a tag mismatch is being handled
			for (;;) {
				switch (source.charAt(tagStart + 1)) {
					case '/':
						...
						if (currentTagName !== tagNameMatch[1]) {
							...
							if (!isHTML || currentTagName.toLowerCase() !== tagNameLower) {
								if (errorsAsTextNodes) {
									// assume the tag is hanging incomplete
									parseStack.pop();
									unclosedTags.pop();
									continue;
								}
								// this is split up due to reported.js:53 WF_ElementTypeMatch_Mismatch and code style reqs
								var matchExpr = '"' + currentTagName + '" != "' + tagNameRaw + '"';
								return errorHandler.fatalError('Opening and ending tag mismatch: ' + matchExpr);
							}
						}
						var config = parseStack.pop();
						unclosedTags.pop();
						var localNSMap = config.localNSMap;
						domBuilder.endElement(config.uri, config.localName, currentTagName, tagStart, source.indexOf('>', tagStart) + 1);
						if (localNSMap) {
					...
					case '>':
						// malformed end tag
						if (errorsAsTextNodes) {
							appendText(tagStart + 1);
							break;
						}
						return errorHandler.fatalError('missing element tag name');
					case '':
						// end of document
						if (errorsAsTextNodes) {
							appendText(tagStart + 1);
						...
						return errorHandler.fatalError('unexpected end of input');
					default:
						if (errorsAsTextNodes) {
							if (source.at(tagStart + 1) === '>') {
								appendText(tagStart + 2);
								break;
							}
						}
						locator && position(tagStart);
						...
						if (locator && len) {
							...
							for (var i = 0; i < len; i++) {
								...
								a.locator.ends = a.endOffset;
							}
							domBuilder.locator = locator2;
							if (appendElement(el, domBuilder, currentNSMap, tagStart, end + 1)) {
								parseStack.push(el);
							}
 ...
 */
function parseElementStartPart(source, start, el, currentNSMap, entityReplacer, errorHandler, isHTML, errorsAsTextNodes) {
	/**
	 * @param {string} qname
	 * @param {string} value
	 * @param {number} startIndex
	 */
	function addAttribute(qname, value, startIndex, endIndex) {
	...
	while (true) {
		...
		switch (c) {
			...
			case '"':
				if (
					...
					if (p > 0) {
						...
						addAttribute(attrName, value, attrStart, p + 1);
						s = S_ATTR_END;
					} else {
			...
			case '/':
				...
				break;
			case '': //end document
				// if errorsAsTextNodes is true, appendText will handle this
				if (errorsAsTextNodes) {
					return p;
				}
				...
				switch (s) {
					...
					case S_ATTR_SPACE:
						...
						if (s == S_ATTR_NOQUOT_VALUE) {
							...
							addAttribute(attrName, value, attrStart, start + value.length);
						} else {
							if (!isHTML) {
								errorHandler.warning('attribute "' + value + '" missed value!! "' + value + '" instead!!');
							}
							addAttribute(value, value, start, p);
						}
						break;
					case S_EQ:
						if (!isHTML) {
				...
				if (c <= ' ') {
					...
					switch (s) {
						...
						case S_ATTR_SPACE:
							...
							addAttribute(attrName, attrName, start, p - 1);
							start = p;
							s = S_ATTR;
							break;
						case S_ATTR_END:
 ...
 */
function appendElement(el, domBuilder, currentNSMap, startOffset, endOffset) {
	...
	domBuilder.startElement(ns, localName, tagName, el, startOffset);
	//endPrefixMapping and startPrefixMapping have not any help for dom builder
	//localNSMap = null
	if (el.closed) {
		domBuilder.endElement(ns, localName, tagName, startOffset, endOffset);
		if (localNSMap) {
	...
	switch (p.char(2)) {
		case '-':
			...
			if (comment) {
				domBuilder.comment(
					comment,
					g.COMMENT_START.length,
					comment.length - g.COMMENT_START.length - g.COMMENT_END.length,
					p.getIndex()
				);
				return p.getIndex();
			} else {
		...
		case '[':
			...
			if (cdata) {
				...
				domBuilder.characters(
					cdata,
					g.CDATA_START.length,
					cdata.length - g.CDATA_START.length - g.CDATA_END.length,
					start,
					p.getIndex()
				);
				domBuilder.endCDATA();
		...
		case 'D': {
			...
			p.skip(1);
			domBuilder.startDTD(doctype.name, doctype.publicId, doctype.systemId, doctype.internalSubset, start, p.getIndex());
			domBuilder.endDTD();
			return p.getIndex();
		}
		default:
...
function parseProcessingInstruction(source, start, domBuilder, errorHandler) {
	...
	var endOfInstr = start + match[0].length;
	domBuilder.processingInstruction(match[1], match[2], start, endOfInstr);
	return endOfInstr;
}
...
ElementAttributes.prototype = {
	...
	addValue: function (qName, value, offset, endOffset) {
		if (!g.QName_exact.test(qName)) {
			throw new Error('invalid attribute:' + qName);
		}
		this.attributeNames[qName] = this.length;
		this[this.length++] = { qName: qName, value: value, offset: offset, endOffset: endOffset };
	},
	length: 0,
