...
function parseDoctypeCommentOrCData(source, start, domBuilder, errorHandler, isHTML) {
	...
	switch (p.char(2)) {
		case '-':
			...
			if (comment) {
				domBuilder.comment(comment, g.COMMENT_START.length, comment.length - g.COMMENT_START.length - g.COMMENT_END.length, p.getIndex());
				return p.getIndex();
			} else {
		...
		case '[':
			...
			if (cdata) {
				...
				domBuilder.characters(cdata, g.CDATA_START.length, cdata.length - g.CDATA_START.length - g.CDATA_END.length, start, p.getIndex());
				domBuilder.endCDATA();
				return p.getIndex();
			} else {
