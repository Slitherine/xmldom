 ...
 * @param overwrite
 * If `true`, the function will overwrite properties in the destination object.
 */
function copy(src, dest, overwrite) {
	for (var p in src) {
		if (!overwrite && hasOwn(dest, p)) {
			continue;
		}
		if (hasOwn(src, p)) {
			var srcDesc = Object.getOwnPropertyDescriptor(src, p);
			if (srcDesc) {
				Object.defineProperty(dest, p, srcDesc);
			} else {
				dest[p] = src[p];
			}
		}
...
// IIFE to extend NodeList prototype extended with Array methods and symbols
(function () {
	if (Symbol && 'iterator' in Symbol) {
		NodeList.prototype[Symbol.iterator] = Array.prototype[Symbol.iterator];
	}
	var ArrayPrototypeMembers = Object.getOwnPropertyNames(Array.prototype);
	for (var i in ArrayPrototypeMembers) {
		var member = ArrayPrototypeMembers[i];
		if (typeof Array.prototype[member] === 'function' && !NodeList.prototype[member]) {
			if (member.slice(-6) === 'String') continue;
			if (member === 'constructor') continue;
			if (member === 'length') continue;
			NodeList.prototype[member] = Array.prototype[member];
		}
	}
})();
/**
...
Document.prototype = {
	...
	get childElementCount() {
		return this.documentElement ? 1 : 0;
	},
};
...
};
{
	...
	for (var i = 0; i < props.length; ++i) {
		var prop = props[i];
		var desc = Object.getOwnPropertyDescriptor(Node.prototype, prop);
